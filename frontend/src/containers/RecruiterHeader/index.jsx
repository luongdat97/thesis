    ...
    return (
        <Header className="d-flex" style={{ position: 'fixed', zIndex: 1, width: '100%', padding: '0 100px' }}>
            ...
            <Menu theme="dark" style={{ width: 530 }} mode="horizontal" defaultSelectedKeys={['1']} overflowedIndicator={<i className="fas fa-bars"></i>}	>
                <Menu.Item key="1"><Link to={`/applicant/job-search`}>Tin tuyển dụng</Link></Menu.Item>
                <Menu.Item key="2"><Link to={`/applicant/cv-template`}>Tìm ứng viên</Link></Menu.Item>
                <Menu.Item key="3"><Link to={`/applicant/cv-template`}>Đặt lịch hẹn</Link></Menu.Item>
                
                <Menu.SubMenu key="4" icon={<SettingOutlined />} title="Tài khoản">
                        <Menu.Item key="5"><Link to={`/applicant/individual/saved-job`}></Link>Thông tin cá nhân</Menu.Item>
                        <Menu.Item key="6"><Link to={`/applicant/individual/applied-job`}></Link>Thông tin công ty</Menu.Item>
